...
a./b | a/b | 元素划分
a.^3 | a\*\*3 | 元素取幂
(a>0.5) | (a>0.5) | 其第i行j列的元素为（a_ij> 0.5）的矩阵。Matlab结果是一个0和1的数组。NumPy结果是布尔值的数组False和True。 
...
max(max(a)) | a.max() | 最大元素（对于matlab，ndims（a）<= 2）
...
sort(a) | sort(a) 要么 a.sort() | 对矩阵 a 进行排序
